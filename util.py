...
def gen_multiple_sliding_window(file_list, window_size, z_normalize, train_cut_off_date, col_name):
    ...
    return np.array(X_train), np.array(y_train), np.array(X_test), np.array(y_test), np.array(train_mean), np.array(
        train_std), np.array(test_mean), np.array(test_std), np.array(x_0_train), np.array(x_0_test)
...
def analyze_results(y_test, y_pred, X_test, save_results=True):
    ...
    df_analyze_results.loc[((df_analyze_results['original'] > df_analyze_results['last_x']) & (
    ...
    #     df_analyze_results['original'] * df_analyze_results['predict'])  # .astype(bool)
    # df_analyze_results['diff'] = df_analyze_results['original'] - df_analyze_results['predict']
    df_analyze_results['diff'] = -np.abs(df_analyze_results['predict'] - df_analyze_results['last_x']) / \
                                 df_analyze_results['last_x']
